...
ifneq ($(shell which composer > /dev/null 2>&1; echo $$?),0) 
	...
	# add bash_completion
ifneq ($(shell grep -qs "commandline/bash_completion" $(HOME)/.{profile,bash{rc,_profile}} > /dev/null 2>&1; echo $$?),0)
...
ifneq ($(shell grep -qs "~/bin\|\$$HOME/bin" $(HOME)/.{profile,bash{rc,_profile}} > /dev/null 2>&1; echo $$?),0)
ifeq ($(shell test -f $(HOME)/.bash_profile),0)
